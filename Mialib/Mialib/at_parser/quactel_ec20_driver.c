...
#define MODULE_NAME "quactel_driver"
...
static void ATQICSGP_cmd(const struct AT_Handler * parent)
{
	...
	char buffer[180];
	snprintf(buffer, ELEMENTS(buffer), "AT+QICSGP=1,1,\"%s\",\"%s\",\"%s\",1", modem->user_apn, modem->user_name, modem->user_password);
...
const AT_Command cmd_AT_QIOPEN = { "AT+QIOPEN", QIOPEN_cmd, 0, gen_result_hadler, gen_responce_hadler, 0, NULL };
...
static inline int m_write(network_modem * This, const char *buffer, unsigned int len)
{
	...
	while(send < len)
	{
		...
		{
			doIt_long_ret_if_error(cmd_AT_QISEND);
		}
...
#ifndef USE_QUACTELEC20_DRIVER
#define USE_QUACTELEC20_DRIVER 0
#endif

#if USE_QUACTELEC20_DRIVER
...
#else
void avoid_warnings_quactelec20____(void) // Fake call to make compiler happy
{
	if((USE_QUACTELEC20_DRIVER))
	{
		init(NULL);
